...
class NACA0012AirfoilBase(nn.Module):
    ...
    def get_relaxation_time(self, rho, T, F, Feq):
        ...
        diff = torch.abs(F - Feq) / Feq.clamp_min(1e-8)
        pop_dim = 1 if F.dim() == 4 else 0
        ...
        alpha = torch.where(eps < 1.0, eps.new_tensor(self.alpha1), alpha)
        alpha = torch.where(eps >= 1.0, 1.0 / tau_dl, alpha)
        tau = alpha * tau_dl
        if F.dim() == 4:
    ...
    def get_boundary_distributions(self, rho, ux, uy, T, khi, zetax, zetay):
        ...
        return inlet_F, inlet_G, top_F, top_G, bottom_F, bottom_G

    def enforce_body_and_bc(self, Fi, Gi, Fi_body, Gi_body, inlet_F, inlet_G, top_F, top_G, bottom_F, bottom_G):
        Fi_obs = Fi.clone()
        Gi_obs = Gi.clone()
...
def plot_fields(rho, ux, uy, T, step, output_path, obstacle_mask, title_prefix, Ma0, Re):
...
def main():
    ...
    print(
        ...
        f"rho0={args.rho0}, T0={args.T0}"
    )

    Fi0, Gi0, khi, zetax, zetay = solver.initial_conditions()
    image_dir = os.path.join("images", f"newton_geq_{args.scenario}_C{chord}_Re{Re:.0e}")
    ...
    with torch.no_grad():
        for step in tqdm(range(args.steps), desc="Newton-Geq rollout"):
            ...
            inlet_F, inlet_G, top_F, top_G, bottom_F, bottom_G = solver.get_boundary_distributions(
                rho, ux, uy, T, khi, zetax, zetay
            )
            Fi0, Gi0 = solver.enforce_body_and_bc(
                Fi, Gi, Fi_body, Gi_body, inlet_F, inlet_G, top_F, top_G, bottom_F, bottom_G
            )
