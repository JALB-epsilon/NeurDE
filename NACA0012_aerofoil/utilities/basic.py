...
class AirfoilStage2Dataset(Dataset):
...
def plot_simulation_results(Field_GT, time_value):
    ...
    fig.suptitle(f"Time: {time_value} (Supersonic flow past a NACA0012 aerofoil)", fontsize=16, fontweight='bold')

    plt.tight_layout(rect=[0, 0.03, 1, 0.95]) # Adjust layout to prevent title overlap with colorbar

    # Save to Images directory
    main_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    image_dir = os.path.join(main_dir, 'images', 'NACA0012_aerofoil')
    os.makedirs(image_dir, exist_ok=True)
    plt.savefig(os.path.join(image_dir, f'NACA0012_aerofoil_{time_value}.png'), bbox_inches='tight')
