...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train Stage 1 main-style Geq baseline")
        ...
        if current_loss < max(best_losses):
            ...
            if epochs_since_last_save[max_index] >= args.save_frequency:
                ...
                save_path = os.path.join(
                    ...
                    f"best_model_{args.case}_epoch_{epoch + 1}_top_{max_index + 1}_loss_{current_loss:.3f}.pt",
                )
    ...
    last_model_path = os.path.join(
        param_training["stage1"]["model_dir"],
        f"last_model_{args.case}_epoch_{epochs}_loss_{current_loss:.3f}.pt",
    )
