...
if __name__ == "__main__":
    ...
    if args.pre_trained_path:
        args.pre_trained_path = args.pre_trained_path.replace("Cylinder_faster/", "").replace("Cylinder/", "")
    ...
    if supervision_mode not in {"feq", "geq"}:
        raise ValueError(f"Cylinder_faster stage2 only supports feq or geq supervision right now, got: {supervision_mode}")
    ...
    for epoch in tqdm(range(epochs), desc="Epochs"):
        ...
        for batch_idx, (F_seq, G_seq, Feq_seq, Geq_seq) in enumerate(dataloader):
            ...
            for rollout in range(number_of_rollout):
                ...
                Fi0, Gi0 = cylinder_solver.enforce_Obs_and_BC(
                                            Fi,
                                            Gi,
                                            Fi_obs_cyl,
                                            Gi_obs_cyl,
